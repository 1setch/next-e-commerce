...
const PromoBar = () => {
        ...
        fetch('/api/auth/me')
            .then((res) => {
                if (res.ok) {
                    ...
                    setIsLoggedIn(false);
                    // Появляется через 1 секунду для гостей
    ...
    const togglePromoBar = () => setIsVisible(false);

    // Пока загружается — ничего не показываем
    if (isLoggedIn === null) return null;
    if (!isVisible || isLoggedIn) return null;

    return (
        <div className={`${styles.promo} ${!isVisible ? styles.promoHidden : ''}`}>
